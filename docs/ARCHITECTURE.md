...
`backend/config` configures Django/DRF, PostgreSQL, Celery, Redis channel layer, and Daphne ASGI (HTTP+WebSocket). `backend/apps/accounts` has UUID users plus session cookie auth APIs. `backend/apps/chat` is account-scoped realtime chat: servers, roles (bitfield), channels, DMs, messages (Postgres timeline), invites/bans, read state, and authorized channel WS (`ws/chat/channels/<id>/`). Not world-bound (D017). `backend/apps/worlds` / `characters` include landing/join. `backend/apps/audit` and `events` provide append-only audit and transactional outbox. `/api/v1/health/` and `/api/v1/ready/` as before. AI stub in `backend/common/ai/` still waits on P019. Organizations draft exists but P005 is unfinished; knowledge/world messaging (P017) not done.
