...
`backend/config` configures Django/DRF, PostgreSQL, Celery, Redis channel layer, Daphne ASGI, credentialed CORS (`django-cors-headers`), and default DRF filter backends (`django-filter`, Search, Ordering — D019). `backend/apps/accounts` has UUID users plus session cookie auth APIs. `backend/apps/organizations` has Organization, OrgMembership, OrgInvitation with capability policies and audited membership/leadership services (P005). `backend/apps/knowledge` has Fact (server-only), Claim, KnowledgeEdge, Evidence, ClaimEvidenceLink, Transmission with audited create/mutate/edge services (P006), allowlist projections + intel GET (P007), and share/verify transmission services + HTTP (P008). `backend/apps/chat` is account-scoped realtime chat: servers, roles (bitfield), channels, DMs, messages (Postgres timeline), invites/bans, read state, and authorized channel WS (`ws/chat/channels/<id>/`). Not world-bound (D017). `backend/apps/worlds` / `characters` include landing/join. `backend/apps/audit` and `events` provide append-only audit and transactional outbox. `/api/v1/health/` and `/api/v1/ready/` as before. AI stub in `backend/common/ai/` still waits on P019. World messaging (P017) not done.

`frontend/src` is a Vite + React 19 SPA (D019): `app/` (providers, TanStack Router), `api/` (Axios session client, query keys, `can()`), `features/{home,auth,worlds}/`, `components/{ui,layout}/`, Tailwind v4 + shadcn/ui. Server state via TanStack Query; Zod at API boundaries. Landing checks `/api/v1/health/`. `docker-compose.yml` runs PostgreSQL, Redis, one-shot migrations, API, worker, and frontend locally. The development web server proxies `/api` (and `/ws`) to the backend. Production serving, static admin assets, readiness, proxy headers, and deployment hardening belong to P001/P036 as specified in their tasks.
