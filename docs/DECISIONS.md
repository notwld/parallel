...
| D018 | Authorization is capability-based and backend-authoritative, not classical Admin/Editor/Viewer RBAC. Pipeline: authenticate → world membership → character → object policy → capability → audience projection. Domains keep local policies (`organizations`, `knowledge`, `chat` bitfields). Frontend mirrors server `allowed_actions` / capability lists for UX only; never trusts client role labels or localStorage privilege flags. External policy engines (Casbin/Oso) deferred until rules outgrow string caps + bitfields. | Spec 2.3, 12.2–12.3; user 2026-09-15. |
| D019 | Frontend SPA stack: Vite + React 19 + TypeScript, Tailwind v4 + shadcn/ui (radix), Axios (`withCredentials` + CSRF), TanStack Query, TanStack Router, Zod, React Hook Form. Feature folders under `frontend/src/features/*`; shared API/authz under `src/api`; shadcn under `src/components/ui`. Zustand deferred until real client-only state exists. Backend list filtering: `django-filter` + DRF `SearchFilter`/`OrderingFilter` after permission selectors; cursor pagination for feeds when those APIs land. | User 2026-09-15; SPA/DRF practice. |
